...
describe "making sure the customer is old enough" do
  ...
  context "when you're a kid" do
    before(:each) do
      @p = Person.new # this should have an attribute '9' for child
      @p.age = 9
    end
    
    it "lets you buy cola and canned haggis" do
      lambda { @nanomart.sell_me(@p, :cola)          }.should_not raise_error
      lambda { @nanomart.sell_me(@p, :canned_haggis) }.should_not raise_error
    end

    it "stops you from buying anything age-restricted" do
      lambda { @nanomart.sell_me(@p, :beer)       }.should raise_error(Nanomart::NoSale)
      lambda { @nanomart.sell_me(@p, :whiskey)    }.should raise_error(Nanomart::NoSale)
      lambda { @nanomart.sell_me(@p, :cigarettes) }.should raise_error(Nanomart::NoSale)
    end
  end 
    
  context "when you're a newly-minted adult" do
    before(:each) do
      @p = Person.new # this should have an attribute '19' for teen
      @p.age = 19
    end
    ...
    it "lets you buy cola, canned haggis, and cigarettes (to hide the taste of the haggis)" do
      lambda { @nanomart.sell_me(@p, :cola)          }.should_not raise_error
      lambda { @nanomart.sell_me(@p, :canned_haggis) }.should_not raise_error
      lambda { @nanomart.sell_me(@p, :cigarettes)    }.should_not raise_error
    end
  ...
  context "when you're an old fogey on Thursday" do
    ...
    it "lets you buy everything" do
      ...
      lambda { @nanomart.sell_me(@p, :canned_haggis) }.should_not raise_error
      lambda { @nanomart.sell_me(@p, :cigarettes)    }.should_not raise_error
      lambda { @nanomart.sell_me(@p, :beer)          }.should_not raise_error
      lambda { @nanomart.sell_me(@p, :whiskey)       }.should_not raise_error
    end
  end
