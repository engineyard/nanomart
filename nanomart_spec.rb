...
describe "making sure the customer is old enough" do
  ...
  context "when you're a kid" do
    before(:each) do
      @customer = Customer.new(9)
    end

    it "lets you buy cola and canned haggis" do
      lambda { @nanomart.sell_me(:cola, @customer)          }.should_not raise_error
      lambda { @nanomart.sell_me(:canned_haggis, @customer) }.should_not raise_error
    end

    it "stops you from buying anything age-restricted" do
      lambda { @nanomart.sell_me(:beer, @customer)       }.should raise_error(Nanomart::NoSale)
      lambda { @nanomart.sell_me(:whiskey, @customer)    }.should raise_error(Nanomart::NoSale)
      lambda { @nanomart.sell_me(:cigarettes, @customer) }.should raise_error(Nanomart::NoSale)
    end
  end

  context "when you're a newly-minted adult" do
    before(:each) do
      @customer = Customer.new(19)
    end

    it "lets you buy cola, canned haggis, and cigarettes (to hide the taste of the haggis)" do
      lambda { @nanomart.sell_me(:cola, @customer)          }.should_not raise_error
      lambda { @nanomart.sell_me(:canned_haggis, @customer) }.should_not raise_error
      lambda { @nanomart.sell_me(:cigarettes, @customer)    }.should_not raise_error
    end
  ...
  context "when you're an old fogey on Thursday" do
    ...
    it "lets you buy everything" do
      ...
      lambda { @nanomart.sell_me(:canned_haggis, @customer) }.should_not raise_error
      lambda { @nanomart.sell_me(:cigarettes, @customer)    }.should_not raise_error
      lambda { @nanomart.sell_me(:beer, @customer)          }.should_not raise_error
      lambda { @nanomart.sell_me(:whiskey, @customer)       }.should_not raise_error
    end
  end
